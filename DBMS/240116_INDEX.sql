...
-- > 실무에서 운영하는 테이블에서는 인덱스의 사용 여부에 따라 성능 차이가 날 수 있고, 대용량의 테이블일 경우에는 
-- > 그 차이가 더욱 커짐
...
-- > SELECT 가 아닌 데이터의 변경작업이 자주 일어나면 오히려 성능이 나빠질 수 있음
...
-- > 책에 색인을 만든다고 해서 책의 본문이 변경되는것은 아닌 것 처럼 보조 인덱스를 만든다고 해서 데이터의 순서나 내용이
-- > 바뀌지는 않음
DROP TABLE IF EXISTS buy, member;
CREATE TABLE member
( mem_id		CHAR(8),
...
-- 인덱스의 내부 작동 원리
-- > 인덱스가 늘 좋은 것은 아니므로 인덱스의 내부 작동 원리를 이해하고 인덱스를 사용해야할 경우와

...
-- NULL값 검색(IS NULL)
-- > member 테이블에서 phone1이 null인 데이터 검색
SELECT *
FROM member
WHERE phone1 = null;

-- "=" 연산자로는 null을 검색할 수 없음
-- null값을 검색할 때는 IS NULL 연산자를 사용
SELECT *
FROM member
...
-- -- > SQL Server에서는 LIMIT과 비슷한 TOP 명령어를 사용
...
-- 연습문제
-- 1. member 테이블에서 회원들을 height의 오름차순으로 조회하기
SELECT *
FROM member
ORDER BY height ASC;
...
-- 5. 동물 보호소에 들어온 동물 중 intake_condition 이 sick인 동물의 데이터를 animal_id의 오름차순으로 조회하기

...
-- 뷰의 실제 작동
-- > 뷰의 실제 생성, 수정, 삭제
-- > 뷰를 생성하면서 뷰에서 사용될 열 이름을 테이블과 다르게 지정할 수도 있음(별칭을 사용해서)
...
DESC member;
-- 에러 발생
-- > v_member뷰가 참조하는 member테이블의 열 중에서 mem_number 열은 NOT NULL 열이라서 반드시
-- > 값이 입력되어야 하지만 현재의 v_member 에서는 mem_number 열을 참조하고 있지 않으므로 값을 입력할 방법이 없음
...
-- 뷰를 통한 데이터 입력
-- > v_height167 뷰에 height가 167미만인 데이터 입력
...
-- > 위와 같은 경우 WITH CHECK OPTION 을 통해 뷰에 설정된 값의 범위가 벗어나는 값은 입력되지 않도록 할 수 있음
...
-- >이제 height가 167미만인 데이터는 입력되지 않고 167이상의 데이터만 입력됨

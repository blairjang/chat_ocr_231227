...
-- 정규화는 단계적으로 실시하기 때문에 첫 번째로 제1 정규화를 시행하고 제1 정규형 테이블을 만듦
...
-- > 제1 정규화를 수행하면 하나의 셀에 하나의 값만 저장하게되므로 정상적으로 테이블화 할 수 있음
-- > 제1 정규화 과정에서 열이 두 개 더 추가되고 행도 늘어나게 됨
-- -- 하나의 셀에 하나의 값만 저장될 수 있도록 하고, 반복되는 부분을 행으로 늘려나가는 것이 제1 정규화

-- > 제1 정규화에서는 중복을 제거하는 테이블의 분할도 이루어짐
-- -- 예제 상황에서는 한 번의 주문으로 여러 개의 상품을 주문할 수 있으므로
...
-- 제2 정규형
-- > 제1 정규화에서 테이블에 기본키를 작성한 것과 같은 방법으로, 데이터가 중복되는 부분을 찾아내어 테이블로 분할
-- > 이때 기본키에 의해 특정되는 열과 그렇지 않은 열로 나누는 것으로 정규화

...
-- -- -- --      다른 테이블에서는 여러 개의 값이 존재할 수 있는 관계
...
-- 입양을 간 기록은 있는데, 보호소에 들어온 기록이 없는 동물의 id와 이름을 id의 오름차순으로 조회하기
...
FROM aac_outcomes outs
	INNER JOIN aac_intakes ins
	ON ins.animal_id = outs.animal_id and outs.datetime <= ins.datetime
ORDER BY ins.datetime ASC;
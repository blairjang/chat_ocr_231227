-- SQL 프로그래밍
-- > SQL 은 다른 프로그래밍 언어와는 많이 다르지만 MYSQL에서 프로그래밍 기능이 필요하다면
-- > 스토어드 프로시저를 사용할 수 있음
...
-- 두 문장 이상이 처리되어야 한다면 BEGIN ~ END 로 묶어야 함
-- 관습적으로는 한 문장이더라도 나중에 추가될 수 있으니 BEGIN ~ END로 묶음
DROP PROCEDURE IF EXISTS ifProc1; -- 만약 기존에 ifProc1()을 만든 적이 있다면 삭제
...
BEGIN
	DECLARE myNum INT; -- myNum 변수 선언. 변수의 데이터형식은 INT
	SET myNum = 200; -- myNum 변수에 200을 대입
	IF myNum = 100 THEN -- myNum 이 100인지 아닌지 구분
		SELECT '100입니다.';
	ELSE
		SELECT '100이 아닙니다.';
	END IF;
...
-- IF문의 활용
-- > 아이디가 APN인 회원의 데뷔 일자가 5년이 넘었는지 확인해보고 5년이 넘었으면 축하 메시지를 출력
...
-- > 조건이 여러 개라면 WHEN 을 여러 번 반복하고 모든 조건에 해당하지 않으면 마지막 ELSE 부분을 수행
...
BEGIN
	...
	WHILE (i <= 100) DO
		IF (i % 4 = 0) THEN
			SET i = i + 1;
			ITERATE myWhile; -- 지정한 label로 돌아가서 계속 진행
		END IF;
...
-- -- 실행한 후에는 DEALLOCATE PREPARE 로 문장을 해제
PREPARE myQuery FROM 'SELECT * FROM member WHERE mem_id = "BLK"';
EXECUTE myQuery; -- 실행이 필요한 시점에서 EXECUTE myQuery 로 실행
...
-- > 보안이 중요한 출입문에서는 출입한 내역을 테이블에 기록하는데

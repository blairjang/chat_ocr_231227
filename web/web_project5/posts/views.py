...
@require_POST # 댓글 작성을 처리할 View, POST요청만 허용
def comment_add(request):
    # request.POST 로 전달된 데이터 사용해 CommentForm 인스턴스 생성
    ...
    if form.is_valid():
        ...
        # url로 "next"값을 전달받았다면 댓글 작성 완료 후 전달받은 값으로 이동
...
def comment_delete(request, comment_id):
    ...
    if comment.user == request.user:
        ...
        return HttpResponseForbidden("댓글을 삭제할 권한이 없습니다.")
    

def post_add(request):
    if request.method == "POST":
        # request.POST로 온 데이터("content" > form.py에서 사용됨)는 PostForm으로 처리
        form = PostForm(request.POST)
        
        if form.is_valid():
            # Post의 "user" 값은 request에서 가져와 자동 할당한다
            post = form.save(commit=False) #commit=False - 변수로만 저장함
            post.user = request.user
            post.save()

            # 포스트 생성한 후 request.FILES.getlist("images")로 전송된 이미지들을 순회하며
            # PostImages 객체를 생성
...
def tags(request, tag_name):
    try:
        ...
        # tag_name에 해당하는 HashTag를 찾지 못한 경우 빈 QuerySet  반환
        posts = Post.objects.none()

    else:    
        # tags(M2M 필드) 찾은 HashTag 객체가 있는 Post를 필터
...
def post_like(request,post_id):
    ...
    # next로 값이 전달되었다면 해당위치로, 전달되지 않았다면 
    # 피드페이지에서 해당 post위치로 이동
